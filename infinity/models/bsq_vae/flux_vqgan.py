...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        if not self.use_checkpoint:
            ...
            return self._forward_with_checkpoint(z)
            # return checkpoint.checkpoint(self._forward, z, use_reentrant=False)

    def _forward_with_checkpoint(self, z: Tensor) -> Tensor:
        def part1(z):
            # z to block_in
            h = self.conv_in(z)
            ...
            h = self.mid.block_2(h)
            return h
        h = checkpoint.checkpoint(part1, z)

        # upsampling
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                def part2(h, i_level, i_block):
                    h = self.up[i_level].block[i_block](h)
                    if len(self.up[i_level].attn) > 0:
                        h = self.up[i_level].attn[i_block](h)
                    return h
                h = checkpoint.checkpoint(part2, h, i_level, i_block)
