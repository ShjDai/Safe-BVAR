...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if vae_type != 0:
                ...
                # print('codes:', codes)
                # print('in ar infer cfg, codes:', codes.shape)
                if si != num_stages_minus_1:
                    ...
                    # print('b')
                    summed_codes += codes if not stega else (stega_codes*stega_mask + (1-stega_mask)*codes)
            else:
                if si < gt_leak:
                    idx_Bl = gt_ls_Bl[si]
        ...
        if vae_type != 0:
            ...
            ret_summed_codes = summed_codes.squeeze(-3).clone()
        else:
            # print('bbb')
        ...
        if stega:
            return ret, idx_Bl_list, img, ret_summed_codes
        else:
            return ret, idx_Bl_list, img
    
    @torch.no_grad()
    def set_kv_cache(self, inference_mode: bool, state: bool):
        if inference_mode:
            for b in self.unregistered_blocks: (b.sa if isinstance(b, CrossAttnBlock) else b.attn).kv_caching(state)
        else:
            assert self.num_block_chunks > 1
            for block_chunk_ in self.block_chunks:
                for module in block_chunk_.module.module:
                    (module.sa if isinstance(module, CrossAttnBlock) else module.attn).kv_caching(state)
    
    @torch.no_grad()
    def predict_next_stage(self, last_stage, cond_BD_or_gss, ca_kv, attn_fn, scale_schedule, abs_cfg_insertion_layers, B, si, cfg, need_to_pad):
        layer_idx = 0
        for block_idx, b in enumerate(self.block_chunks):
            # last_stage shape: [4, 1, 2048], cond_BD_or_gss.shape: [4, 1, 6, 2048], ca_kv[0].shape: [64, 2048], ca_kv[1].shape [5], ca_kv[2]: int
            if self.add_lvl_embeding_only_first_block and block_idx == 0:
    ...
    @torch.no_grad()
    def get_idx_Bld(self, last_stage, cond_BD, cfg, add_cfg_on_logits, tau_list, si, B, top_k, top_p, rng, gt_leak, pn, gt_ls_Bl):
        if (cfg != 1) and add_cfg_on_logits:
            # print(f'add cfg on add_cfg_on_logits')
            logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si])
            logits_BlV = cfg * logits_BlV[:B] + (1-cfg) * logits_BlV[B:]
    ...
    @torch.no_grad()
    def autoregressive_infer_edit_cfg(
        self,
        vae=None,
        scale_schedule=None,
        label_B_or_BLT=None,
        ...
        self.set_kv_cache(inference_mode, True)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if si >= trunk_scale:
                break
            # print(f'at stage {si}, scale: {pn}, "\
                #   "summed_codes.shape: {summed_codes.shape if isinstance(summed_codes, torch.Tensor) else -1}, last_stage.shape: {last_stage.shape}')
            attn_fn = self.attn_fn_compile_dict.get(tuple(scale_schedule[:(si+1)]), None) if self.use_flex_attn else None
            
            last_stage = self.predict_next_stage(last_stage, cond_BD_or_gss, ca_kv, attn_fn, scale_schedule, abs_cfg_insertion_layers, B, si, cfg, 0)
            idx_Bld = self.get_idx_Bld(last_stage, cond_BD, cfg, add_cfg_on_logits, tau_list, si, B, top_k, top_p, rng, gt_leak, pn, gt_ls_Bl)
            ...
            if si != num_stages_minus_1:
                ...
                summed_codes += codes
            summed_codes_list.append(summed_codes.clone())
            if si != num_stages_minus_1:
                last_stage = self.word_embed(self.norm0_ve(last_stage))
                last_stage = last_stage.repeat(bs//B, 1, 1)
            
            stages.append(last_stage.clone())

        self.set_kv_cache(inference_mode, False)
        img = vae.decode(summed_codes.squeeze(-3))
        img = (img + 1) / 2
        img = img.permute(0, 2, 3, 1).mul_(255).to(torch.uint8).flip(dims=(3,))
        return stages, img, summed_codes_list
    
    @torch.no_grad()
    def transform_last_stage(self, last_stage, bs, B):
        last_stage = last_stage.squeeze(-3) # [B, d, h, w] or [B, d, 2h, 2w]
        if self.apply_spatial_patchify: # patchify operation
            last_stage = torch.nn.functional.pixel_unshuffle(last_stage, 2) # [B, 4d, h, w]
        last_stage = last_stage.reshape(*last_stage.shape[:2], -1) # [B, d, h*w] or [B, 4d, h*w]
    ...
    @torch.no_grad()
    def autoregressive_edit_from_si(
        self,
        vae=None,
        scale_schedule=None,
        label_B_or_BLT=None,
        ...
        returns_vemb=0,cfg_insertion_layer=[-5],
        vae_type=0, gt_leak=0, gt_ls_Bl=None,
        inference_mode=False, from_si=0, given_stage=None, given_summed_codes=None, **kwargs
    ):
        if g_seed is None: rng = None
        else: self.rng.manual_seed(g_seed); rng = self.rng
        assert len(cfg_list) >= len(scale_schedule)
        ...
        summed_codes = 0
        stages = []
        if given_stage is None or given_summed_codes is None:
            last_stage = sos.unsqueeze(1).expand(bs, 1, -1) + self.pos_start.expand(bs, 1, -1)
        else:
            last_stage = given_stage
            summed_codes = given_summed_codes
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            # print(f'at stage {si}, scale: {pn}, summed_codes.shape: {summed_codes.shape}, last_stage.shape: {last_stage.shape}')
            # summed_codes, last_stage, idx_Bld = self.inference_from_stage(vae, summed_codes, last_stage, scale_schedule, cond_BD_or_gss, ca_kv, cond_BD, cfg_list, tau_list, top_k, top_p, rng, gt_leak, pn, gt_ls_Bl, B, si, abs_cfg_insertion_layers, add_cfg_on_logits, add_cfg_on_probs, num_stages_minus_1, vae_scale_schedule, bs)
            cfg = cfg_list[si]
            attn_fn = self.attn_fn_compile_dict.get(tuple(scale_schedule[:(si+1)]), None) if self.use_flex_attn else None
            ...
            if si != num_stages_minus_1:
                summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                last_stage = F.interpolate(summed_codes, size=vae_scale_schedule[si+1], mode=vae.quantizer.z_interplote_up) # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                last_stage = self.transform_last_stage(last_stage, bs, B)
            else:
                summed_codes += codes
            
            del kv_compact, lens, cu_seqlens_k, max_seqlen_k, idx_Bld, codes
            # summed_codes_list.append(summed_codes.clone())
            # stages.append(last_stage.clone())
    ...
    @torch.no_grad()
    def inference_from_stage(self, vae, summed_codes, last_stage, scale_schedule, cond_BD_or_gss, ca_kv, cond_BD, cfg_list, tau_list, top_k, top_p, rng, gt_leak, pn, gt_ls_Bl, B, si, abs_cfg_insertion_layers, add_cfg_on_logits, add_cfg_on_probs, num_stages_minus_1, vae_scale_schedule, bs):
        cfg = cfg_list[si]
        attn_fn = self.attn_fn_compile_dict.get(tuple(scale_schedule[:(si+1)]), None) if self.use_flex_attn else None
        ...
        if si != num_stages_minus_1:
            ...
            summed_codes += codes
        return summed_codes, last_stage, idx_Bld
