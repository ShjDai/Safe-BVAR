...
def gen_one_img(
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        ...
        _, _, img_list = infinity_test.autoregressive_infer_cfg(
            ...
            sampling_per_bits=sampling_per_bits, stega_codes=stega_codes, stega_mask=stega_mask
        )
...
def gen_one_img_ste(
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        stt = time.time()
        _, _, img_list, codes = infinity_test.autoregressive_infer_cfg(
            vae=vae,
            scale_schedule=scale_schedule,
            label_B_or_BLT=text_cond_tuple, g_seed=g_seed,
            B=1, negative_label_B_or_BLT=negative_label_B_or_BLT, force_gt_Bhw=None,
            ...
            stega=True, stega_codes=stega_codes, stega_mask=stega_mask
        )
    print(f"cost: {time.time() - sstt}, infinity cost={time.time() - stt}")
    img = img_list[0]
    return img, codes
...
def load_tokenizer(t5_path ='', device='cuda'):
    ...
    text_tokenizer: T5TokenizerFast = AutoTokenizer.from_pretrained(t5_path, revision=None, legacy=True, cache_dir='/root/autodl-tmp/repo/Infinity-latest/cache')
    text_tokenizer.model_max_length = 512
    text_encoder: T5EncoderModel = T5EncoderModel.from_pretrained(t5_path, torch_dtype=torch.float16, cache_dir='/root/autodl-tmp/repo/Infinity-latest/cache')
...
def load_visual_tokenizer(args, cuda_device='cuda'):
